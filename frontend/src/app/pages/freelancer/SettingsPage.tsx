    ...
    <DashboardLayout userType="freelancer">
      ...
      <div className="space-y-6 max-w-4xl">
        ...
        <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-8">
          ...
          <p className="text-sm text-[#888888] mb-6">Portfolio yang disimpan di sini otomatis tampil di profile publik freelancer.</p>

          <div className="grid md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-sm text-[#888888] mb-2">Judul Portfolio</label>
              <input
                value={portfolioForm.title}
                onChange={(event) => setPortfolioForm({ ...portfolioForm, title: event.target.value })}
                placeholder="Contoh: Wedding Outdoor Surabaya"
                className="w-full bg-[#141414] border border-[#2A2A2A] rounded-lg px-4 py-3 text-white placeholder-[#888888] focus:border-[#F5C800] focus:outline-none"
              />
            </div>
            <div>
              <label className="block text-sm text-[#888888] mb-2">Kategori / Jasa</label>
              <input
                value={portfolioForm.category}
                onChange={(event) => setPortfolioForm({ ...portfolioForm, category: event.target.value, serviceType: event.target.value })}
                placeholder="Contoh: Wedding, Product, Event"
                className="w-full bg-[#141414] border border-[#2A2A2A] rounded-lg px-4 py-3 text-white placeholder-[#888888] focus:border-[#F5C800] focus:outline-none"
              />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm text-[#888888] mb-2">Deskripsi</label>
              <textarea
                value={portfolioForm.description}
                onChange={(event) => setPortfolioForm({ ...portfolioForm, description: event.target.value })}
                rows={3}
                placeholder="Ceritakan singkat tentang hasil kerja ini"
                className="w-full bg-[#141414] border border-[#2A2A2A] rounded-lg px-4 py-3 text-white placeholder-[#888888] focus:border-[#F5C800] focus:outline-none"
              />
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 mb-4">
            <label className="inline-flex items-center gap-2 px-4 py-3 border border-[#888888] text-white rounded-lg hover:border-[#F5C800] hover:text-[#F5C800] cursor-pointer transition-colors">
              ...
              <input
                ...
                accept="image/png,image/jpeg"
                className="hidden"
                onChange={(event) => uploadPortfolioFile(event.target.files?.[0])}
              />
            </label>
            ...
            {portfolioForm.previewUrl && (
              <button
                type="button"
                onClick={() => setPortfolioForm((current) => ({
                  ...current,
                  fileUrl: '',
                  previewUrl: '',
                  fileName: '',
                ...
                className="inline-flex items-center gap-1 text-[#EF4444] text-sm"
              >
                <X className="w-4 h-4" />
                Hapus file
              </button>
          ...
          {portfolioForm.previewUrl && (
            <img src={portfolioForm.previewUrl} alt={portfolioForm.fileName} className="mb-4 max-h-64 rounded-lg object-contain bg-[#141414]" />
          )}
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-3 bg-[#F5C800] text-black font-bold rounded-lg hover:shadow-[0_0_20px_rgba(245,200,0,0.4)] transition-all disabled:opacity-60"
          >
            <Plus className="w-4 h-4" />
            {portfolioSaving ? 'Menyimpan...' : 'Tambah Portfolio'}
          </button>

          <div className="grid md:grid-cols-2 gap-4 mt-6">
            {portfolioItems.length === 0 && (
              <p className="text-sm text-[#888888] md:col-span-2">Belum ada portfolio. Tambahkan contoh hasil kerja agar client lebih yakin.</p>
            )}
            {portfolioItems.slice(0, 4).map((item) => (
              <div key={item.id} className="bg-[#141414] border border-[#2A2A2A] rounded-lg p-4">
                {item.fileUrl && (
                  <img src={item.fileUrl} alt={item.title} className="w-full h-40 object-cover rounded-lg mb-3 bg-[#0A0A0A]" />
                )}
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="font-bold text-white">{item.title}</div>
                    <div className="text-sm text-[#888888]">{item.serviceType || item.category || 'Portfolio'}</div>
                  </div>
                  <button
                    type="button"
        ...
        <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-8">
          ...
          <div className="mb-5">
            <div className="text-sm text-[#888888] mb-2">Pakai contoh cepat</div>
            <div className="flex flex-wrap gap-2">
              {offeringTemplates.map((template) => (
                <button
                  key={template.label}
                  type="button"
                  onClick={() => setOfferingForm((current) => ({
                    ...current,
                    ...template,
                    customServiceType: '',
                    description: current.description,
                  }))}
                  className="px-3 py-2 rounded-full bg-[#141414] border border-[#2A2A2A] text-[#888888] text-sm hover:border-[#F5C800] hover:text-[#F5C800] transition-colors"
                >
                  {template.label}
                </button>
              ))}
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-sm text-[#888888] mb-2">Jenis Jasa</label>
              <select
            ...
            {offeringForm.serviceType === 'Lainnya' && (
              <div>
                <label className="block text-sm text-[#888888] mb-2">Jenis Jasa Custom</label>
                <input
                  value={offeringForm.customServiceType}
                  onChange={(event) => setOfferingForm({ ...offeringForm, customServiceType: event.target.value })}
                  className="w-full bg-[#141414] border border-[#2A2A2A] rounded-lg px-4 py-3 text-white focus:border-[#F5C800] focus:outline-none"
                />
              </div>
            )}
            <div>
              <label className="block text-sm text-[#888888] mb-2">Nama Tampilan Jasa</label>
              <input
                value={offeringForm.title}
                onChange={(event) => setOfferingForm({ ...offeringForm, title: event.target.value })}
                placeholder="Contoh: Dokumentasi Wedding Premium"
                className="w-full bg-[#141414] border border-[#2A2A2A] rounded-lg px-4 py-3 text-white placeholder-[#888888] focus:border-[#F5C800] focus:outline-none"
              />
            </div>
            <div>
              <label className="block text-sm text-[#888888] mb-2">Harga Per Jam (Rp) - wajib</label>
              <input
                type="number"
                value={offeringForm.ratePerHour}
                onChange={(event) => setOfferingForm({ ...offeringForm, ratePerHour: event.target.value })}
                placeholder="Contoh demo: 1 atau 150000"
                className="w-full bg-[#141414] border border-[#2A2A2A] rounded-lg px-4 py-3 text-white focus:border-[#F5C800] focus:outline-none"
              />
              <p className="text-xs text-[#888888] mt-2">Ini nominal harga, bukan jumlah jam. Untuk demo, Rp 1 diperbolehkan.</p>
            </div>
            <div>
              <label className="block text-sm text-[#888888] mb-2">Rate Per Foto (Opsional)</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm text-[#888888] mb-2">Biaya Per Orang Tambahan</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm text-[#888888] mb-2">Estimasi Durasi (Jam)</label>
              <select
              ...
              <p className="text-xs text-[#888888] mt-2">Ini jumlah jam kerja default untuk kalkulasi otomatis.</p>
            </div>
            <div>
              <label className="block text-sm text-[#888888] mb-2">Kapasitas Maksimal Tim</label>
              <input
                type="number"
                min="1"
          ...
          <div className="mb-4">
            <label className="block text-sm text-[#888888] mb-2">Tag / Label Jasa</label>
            <div className="flex flex-wrap gap-2">
              {[...new Set([...tagOptions, ...offeringForm.relatedSpecs])].map((tag) => (
                <button
                  key={tag}
                  type="button"
            ...
            <div className="flex gap-2 mt-3">
              <input
                ...
                placeholder="Tambah tag custom, contoh: Graduation"
              ...
              <button
                ...
                className="px-4 py-3 bg-[#F5C800] text-black rounded-lg font-bold"
              >
                Add
              </button>
            </div>
            {offeringForm.relatedSpecs.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-3">
                {offeringForm.relatedSpecs.map((tag) => (
                  <span key={tag} className="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-[#F5C800]/10 text-[#F5C800] text-sm">
                    #{tag}
                    <button type="button" onClick={() => toggleOfferingTag(tag)} aria-label={`Hapus tag ${tag}`}>
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                ))}
          ...
          <div className="mb-4">
            <label className="block text-sm text-[#888888] mb-2">Deskripsi / Benefit</label>
            <textarea
              value={offeringForm.description}
              onChange={(event) => setOfferingForm({ ...offeringForm, description: event.target.value })}
              rows={4}
              placeholder="Tulis benefit per baris agar mudah dibaca client"
              className="w-full bg-[#141414] border border-[#2A2A2A] rounded-lg px-4 py-3 text-white placeholder-[#888888] focus:border-[#F5C800] focus:outline-none"
            />
          </div>
          ...
          <div className="mt-6 space-y-3">
            {offerings.length === 0 && (
              <p className="text-sm text-[#888888]">Belum ada jasa. Tambahkan minimal satu agar client bisa memesan dari profile.</p>
            )}
            {offerings.map((offering) => (
              <div key={offering.id} className="bg-[#141414] border border-[#2A2A2A] rounded-lg p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="font-bold text-white">{offering.title || offering.serviceType}</div>
                    <div className="text-sm text-[#888888]">
                      {offering.serviceType} - {offering.ratePerHourFormatted}/jam - kapasitas {offering.capacityPersons || 1} orang
                    </div>
