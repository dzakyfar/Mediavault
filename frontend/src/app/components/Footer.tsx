...
export const footerLinks = {
  // Ganti nilai ini dengan URL Instagram resmi MediaVault saat sudah siap deploy.
  instagram: 'https://www.instagram.com/mediavault.studio',
  community: 'https://chat.whatsapp.com/FZea3gGJyhsAthTOdpZW7C',
};

export default function Footer() {
  const { t } = useLanguage();
  const [showContact, setShowContact] = useState(false);
  const contacts = ['nathanael777@gmail.com', 'dzakyfardya@gmail.com', 'fauzanalwahyu827@gmail.com'];
  const scrollToTop = () => window.scrollTo({ top: 0, left: 0, behavior: 'smooth' });

  useEffect(() => {
    const openContact = () => setShowContact(true);
    window.addEventListener('mediavault:open-contact', openContact);
    return () => window.removeEventListener('mediavault:open-contact', openContact);
  }, []);
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-4 gap-12 mb-12">
            <div>
              ...
              <p className="text-[#888888] text-sm mb-4">{t('Pesan. Produksi. Kirim.', 'Book. Shoot. Deliver.')}</p>
              <p className="text-[#888888] text-sm">{t('Platform kreatif untuk jasa fotografi dan video dengan workflow yang rapi.', "Indonesia's boldest platform for creative photography and video services.")}</p>
            </div>
            <div>
              <h4 className="font-bold mb-4 text-white">{t('Perusahaan', 'Company')}</h4>
              <ul className="space-y-2 text-[#888888]">
                <li><Link to="/about" onClick={scrollToTop} className="hover:text-[#F5C800] transition-colors">{t('Tentang Kami', 'About Us')}</Link></li>
                <li>
                  <button
                    type="button"
            ...
            <div>
              <h4 className="font-bold mb-4 text-white">{t('Untuk Klien', 'For Clients')}</h4>
              <ul className="space-y-2 text-[#888888]">
                <li><Link to="/post-job" onClick={scrollToTop} className="hover:text-[#F5C800] transition-colors">{t('Buat Pekerjaan', 'Post a Job')}</Link></li>
                <li><Link to="/explore" onClick={scrollToTop} className="hover:text-[#F5C800] transition-colors">{t('Cari Talent', 'Search Talent')}</Link></li>
              </ul>
            </div>
            <div>
              <h4 className="font-bold mb-4 text-white">{t('Untuk Freelancer', 'For Freelancers')}</h4>
              <ul className="space-y-2 text-[#888888]">
                <li><Link to="/success-stories" onClick={scrollToTop} className="hover:text-[#F5C800] transition-colors">{t('Cerita Sukses', 'Success Stories')}</Link></li>
                <li>
                  <a href={footerLinks.community} target="_blank" rel="noreferrer" className="hover:text-[#F5C800] transition-colors">
          ...
          <div className="flex items-center justify-center gap-6 mb-8">
            <a href={footerLinks.instagram} target="_blank" rel="noreferrer" className="text-[#888888] hover:text-[#F5C800] transition-colors">Instagram</a>
          </div>
          <div className="text-center text-[#888888] text-sm">
            {t('Hak cipta 2026 MediaVault. Dibuat di Surabaya.', 'Copyright 2026 MediaVault. Made in Surabaya.')}
          </div>
        </div>
      </footer>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
          ...
          <div className="relative w-full max-w-lg overflow-hidden rounded-3xl border border-[#D9A900]/30 bg-white p-8 text-[#111827] shadow-[0_24px_80px_rgba(15,23,42,0.22)] dark:border-[#F5C800]/30 dark:bg-[#101010] dark:text-white dark:shadow-[0_24px_80px_rgba(0,0,0,0.55)]">
            ...
            <div className="relative">
              ...
              <p className="mb-2 text-sm font-bold uppercase tracking-[0.3em] text-[#A87800] dark:text-[#F5C800]">{t('Kontak', 'Contact')}</p>
